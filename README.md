...
classDiagram
    ...
    class OrderStatus {
        Integer status
        +valueOf(Integer status) OrderStatus
    }

    User "1" -- "0..*" Order : client
    Order "1" -- "0..*" Payment : payment
    Order "1" -- "0..*" OrderItem : items
    OrderItem "1" -- "1" OrderItemPK : id
    OrderItemPK "1" -- "1" Product : product
    OrderItemPK "1" -- "1" Order : order
    Product "0..*" -- "0..*" Category : categories
    Product "1" -- "0..*" OrderItem : items
    Category "0..*" -- "0..*" Product : products
